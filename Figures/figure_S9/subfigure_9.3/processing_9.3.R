...
sample_data <- data.frame(
  sample = c("D1a","D2a", "D3a","D4a","D5a","D6a"),
  condition = c("Darkness_negative_control", "Darkness_negative_control", "Darkness_negative_control", "Darkness_positive_control", "Darkness_positive_control", "Darkness_positive_control"),
...
# METHOD2: WT test
sleuth_obj = sleuth_wt(sleuth_obj, 'conditionDarkness_positive_control')

sleuth_table_WT = sleuth_results(sleuth_obj, 'conditionDarkness_positive_control', test_type = 'wt')
...
ggplot(sleuth_table_WT, aes(x = b/log(2), y = -log10(qval))) +
  ...
  theme_minimal()+
  ggtitle("eGFP-RelA + Cry2olig-mCh-FUSN-NLS-NbGFP + Cry2olig-mCh-FUSN-NLS : Darkness vs Blue Light")+
...
write.table(sleuth_table_WT, file="sleuth_treated_gene_WT.tsv", sep='\t', row.names=FALSE, quote=FALSE)
